...
import { fileToBase64, fetchProducts,selectProducts,selectIsLoading,selectError,addProduct,updateProduct,deleteProduct,ProductItem,} from "@/lib/redux/features/productSlice";
...
export default function ProductsPage() {
  ...
  const handleSave = async (
    ...
    newImages: File[],
    deletedImages: string[] // ✅ Receive this
  ) => {
    try {
      const formPayload = new FormData();
      formPayload.append("name", formData.name);
      formPayload.append("summary", formData.summary);
      formPayload.append("power", formData.power);
  ...
  return (
    <div className="mx-auto p-0 flex flex-col gap-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-2 mb-1 flex-wrap">
        <h2 className="text-xl font-bold text-[var(--primary-green)]">Products</h2>
      ...
      <Dialog open={!!deleteProductData} onOpenChange={(open) => !open && setDeleteProductData(null)}>
        <DialogContent>
          <DialogHeader>
